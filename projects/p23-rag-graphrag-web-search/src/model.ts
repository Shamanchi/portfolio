...
export interface GraphRAGWebConfig {
  corpus: string[];
  entityTypes: string[];
...
export const DEFAULT_CONFIG: GraphRAGWebConfig = {
  corpus: [
...
export function tokenize(text: string): string[] {
  const out: string[] = [];
  for (const raw of text.toLowerCase().match(/[a-z][a-z0-9_-]{1,24}/g) ?? []) {
    for (const piece of raw.split("-")) if (piece.length >= 2) out.push(piece);
  }
  return out;
}
...
export function extractEntities(doc: string, config: GraphRAGWebConfig): Entity[] {
...
export function detectRelations(doc: string, config: GraphRAGWebConfig): string[] {
  ...
  const found: string[] = [];
  for (const tok of config.relationTypes) {
    const hints = markers[tok] ?? [];
    if (hints.some((h) => lower.includes(h))) found.push(tok);
  }
  return found;
}

export function extractRelations(doc: string, config: GraphRAGWebConfig): string[] {
  return detectRelations(doc, config);
}

export function buildGraph(config: GraphRAGWebConfig): KnowledgeGraph {
...
export function localSearch(graph: KnowledgeGraph, entity: string, limit: number): Entity[] {
  ...
  while (queue.length > 0 && relevant.size < limit) {
    const current = queue.shift() as string;
    const linked = neighbors(graph, current);
    for (const name of linked) {
      if (!relevant.has(name) && graph.entities.has(name)) {
        relevant.set(name, graph.entities.get(name) as Entity);
        queue.push(name);
      }
    }
  }
...
export function validateConfig(config: GraphRAGWebConfig): string[] {
  ...
  if (!Number.isInteger(config.searchResultsLimit) || config.searchResultsLimit < 1) problems.push("searchResultsLimit is invalid");
